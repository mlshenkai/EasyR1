...
def load_vllm_with_lora(
        model_name_or_path: str,
        gpus: typing.List[int],
        **vllm_kwargs,
):
    """
    Load vllm model. If the model is a LORA model, load the LORA model.
    """
    # hf checkpoint has a adapter_config.json file if it is a LORA model.
    if pathlib.Path(model_name_or_path).exists() and 'adapter_config.json' in os.listdir(model_name_or_path):
        config = json.load(open(os.path.join(model_name_or_path, 'adapter_config.json'), 'r', encoding='utf-8'))
        ...
        llm = LLM(
            ...
            enable_lora=True,
            # disable_async_output_proc=True, # 0.6.0 bug when preemption happens.
            dtype='bfloat16' if gpu.get_compute_capability()[0] >= 8 else 'half',
            swap_space=0,
            **vllm_kwargs,
        )
        lora_request = LoRARequest('adapter', 1, model_name_or_path)
        llm = functools.partial(llm.generate, lora_request=lora_request)
    else:
        llm = LLM(
            model=model_name_or_path,
            tensor_parallel_size=len(gpus),
            trust_remote_code=True,
            # kv_cache_dtype='fp8', # very bad both in time and accuracy.
            # disable_async_output_proc=True, # 0.6.0 bug when preemption happens.
            dtype='bfloat16' if gpu.get_compute_capability()[0] >= 8 else 'half',
            swap_space=0,
            **vllm_kwargs,
