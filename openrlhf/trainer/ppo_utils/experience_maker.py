...
def preprocess_orm_response(sequence: str) -> str:
    """
    预处理 orm的返回结果
    Args:
        sequence:
...
def preprocess_orm_reward(queries, tokenizer, **generate_kwargs):
    ...
    for input_for_prm in queries:
        ...
        for input_id, input_mask in zip(
            input_token["input_ids"], input_token["attention_mask"]
        ):
    ...
    input_ids = zero_pad_sequences(
        input_ids, side=padding_side, value=tokenizer.pad_token_id
    )
    ...
    def make_experience(self, samples: Samples, **generate_kwargs) -> Experience:
        ...
        if self.remote_rm_url is not None:
            ...
            missing_answer_indices = [
                i for i, query in enumerate(queries) if "\n\n# Answer\n\n" not in query
            ]

            queries = [preprocess_orm_response(query) for query in queries]

            reward_sequences, reward_attention_mask = preprocess_orm_reward(
                queries, self.tokenizer, **generate_kwargs
            )
            ...
            logits = self.reward_model(
                reward_sequences.to(device=action_log_probs.device),
                attention_mask=reward_attention_mask.to(device=action_log_probs.device),
                return_output=True,
            ...
            for index, score in enumerate(scores):
                if index in missing_answer_indices:
                    # 如果 query 不包含 \n\n# Answer\n\n，设置 step_score 为 0.0
                    step_scores.append(
                        torch.tensor(0.0, device=action_log_probs.device)
                    )
                else:
                    # 否则正常计算 step_score
            ...
            min_values = [
                (
                    ...
                    else torch.tensor(-0.5, device=action_log_probs.device)
                )
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        if not self.remote_rm_url:
            ...
            if not self.packing_samples:
                queries = self.tokenizer.batch_decode(
                    sequences_cpu, skip_special_tokens=False
                )
            else:
                sequences_list = []
                offset = 0
                ...
                for length in packed_seq_lens:
                    ...
                    offset += length
                queries = self.tokenizer.batch_decode(
                    sequences_list, skip_special_tokens=False
                )
            ...
            if self.custom_reward_func:
                ...
                for rm in self.remote_rm_url:
                    r = remote_rm_fn_ray.remote(
                        rm, queries=queries, prompts=samples.prompts
                    )
                    r_refs.append(r)
        ...
        action_log_probs = self.actor(
            sequences, num_actions, attention_mask, packed_seq_lens=packed_seq_lens
        )
        ...
        base_action_log_probs, value, rewards = (
            ref_values[0],
            ref_values[1],
            ref_values[2:],
        )
        ...
        for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
            ...
            if not self.packing_samples:
                ...
                for output in outputs:
                    ...
                    max_output_len = max(
                        max_output_len, len(output.outputs[0].token_ids)
                    )
                ...
                for output in outputs:
                    ...
                    output_ids = list(output.outputs[0].token_ids) + [pad_token_id] * (
                ...
                attention_mask = torch.tensor(attention_mask, device="cuda").unsqueeze(
                    0
                )
                action_mask = None
                response_length = torch.tensor(
                    num_actions, device="cuda", dtype=torch.float
                )
                total_length = torch.tensor(
                    packed_seq_lens, device="cuda", dtype=torch.float
                )
                samples_list.append(
                    Samples(
                        sequences=sequences,
