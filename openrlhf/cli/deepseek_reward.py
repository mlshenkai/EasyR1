# -*- coding: utf-8 -*-
# @Author: watcher
# @Created Time: 2025/2/27 15:02
# @File: deepseek_reward
...
def _get_openai_client():
    openai_keys = [
        ...
        if key.lower().startswith("openai_api_key")
    ]
...
def _get_deepseek_client(endpoint_url):
    deepseek_keys = [
        value
        for key, value in os.environ.items()
        if key.lower().startswith("deepseek_api_key")
    ]
    api_key = random.choice(deepseek_keys)
    return openai.OpenAI(
        api_key=api_key, base_url=endpoint_url
    )
...
def _extract_score_from_response(response: str) -> float:
    ...
    try:
        ...
        match = re.search(
            r'(?i)[\'"]?(score|grade)[\'"]?\s*:\s*[\'"]?(\d+(\.\d+)?)[\'"]?', response
        )
        if match:
...
def _get_client(api_type: str, endpoint_url: str = None):
    if api_type == "azure":
        ...
        client = _get_deepseek_client(endpoint_url)
    else:
...
def grade_cot(
    problem: str,
    answer: str,
    grader_model_name: str = "gpt-4o-mini",
    api_type: str = "openai",
    endpoint_url: str = "",
...
if __name__ == "__main__":
    ans = grade_cot(
        "What is 2 + 5?",
        "Primary school math. Adding two apples and five apples. It is seven apples. It is 7.",
        "deepseek-v3",
        "deepseek",
